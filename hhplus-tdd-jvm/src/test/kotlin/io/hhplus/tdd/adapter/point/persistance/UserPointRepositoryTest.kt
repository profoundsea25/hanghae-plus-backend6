...
class UserPointRepositoryTest {
    ...
    @BeforeEach
    fun beforeEach() {
        userPointRepository.save(USER_POINT_1.id, USER_POINT_1.point)
        userPointRepository.save(USER_POINT_2.id, USER_POINT_2.point)
        userPointRepository.save(USER_POINT_3.id, USER_POINT_3.point)
    }
    ...
    fun shouldReturnCorrectUserPointWhenCharged() {
        val inputId: Long = USER_POINT_1.id
        val inputAmount: Long = 20

        val actual: UserPoint = userPointRepository.save(inputId, inputAmount)
